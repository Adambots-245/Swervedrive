...
public class VisionProcessorSubsystem extends SubsystemBase {
    ...
    public void init() {
        Solenoid ringLight = new Solenoid(PneumaticsModuleType.CTREPCM, Constants.VisionConstants.RING_LIGHT_PORT);
        // ringLight.clearAllPCMStickyFaults();
        // if pcm status is flashing orange (sticky fault), run once
        ringLight.set(true);
        camera = CameraServer.startAutomaticCapture(Constants.VisionConstants.CAM_NUMBER);
        camera.setExposureManual(Constants.VisionConstants.CAM_EXPOSURE);
        processedOutputStream = CameraServer.putVideo("Output", Constants.VisionConstants.IMG_WIDTH, Constants.VisionConstants.IMG_HEIGHT);
        processedOutputStream.setVideoMode(PixelFormat.kGray, Constants.VisionConstants.IMG_WIDTH, Constants.VisionConstants.IMG_HEIGHT, Constants.VisionConstants.DRIVER_STATION_FPS);
        processedOutputStream.setFPS(Constants.VisionConstants.DRIVER_STATION_FPS);
        ...
        camera.setVideoMode(VideoMode.PixelFormat.kMJPEG, Constants.VisionConstants.IMG_WIDTH, Constants.VisionConstants.IMG_HEIGHT, Constants.VisionConstants.PROCESSING_FPS);
    ...
    public void drawRect(Point[] pts) {
        for (int i = 0; i < 4; i++)
            Imgproc.line(mat, pts[i], pts[(i + 1) % 4], Constants.VisionConstants.GREEN, 1);
    ...
    public void drawCrosshair() {
        Imgproc.line(mat, new Point(crosshair.x - 5, crosshair.y - 5), new Point(crosshair.x + 5, crosshair.y + 5), Constants.VisionConstants.RED, 3);
        Imgproc.line(mat, new Point(crosshair.x - 5, crosshair.y + 5), new Point(crosshair.x + 5, crosshair.y - 5), Constants.VisionConstants.RED, 3);
    ...
    public void calculateAngle() {
        pixelDistance = (int) crosshair.x - Constants.VisionConstants.IMG_HOR_MID;
        angle = pixelDistance * Constants.VisionConstants.HOR_DEGREES_PER_PIXEL;
